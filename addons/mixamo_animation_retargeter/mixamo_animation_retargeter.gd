...
func RetargetMixamoAnimation(id: int) -> void:
	if id != retarget_menu_id:
		return

	var selected_paths: PackedStringArray = (
		get_editor_interface().get_selected_paths()
	)

	var fbx_files: Array[String] = []
	...
	if fbx_files.is_empty():
		push_warning("Select at least one FBX file in the FileSystem dock.")
		return
